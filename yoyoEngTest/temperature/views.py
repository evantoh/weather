...
def weather_detail(request):
    ...
    weather_data = get_weather(location, days)  # Call to get_weather function which returns a formatted dictionary

    #  Using Low-level cache API
    if cache_data := cache.get("{}-{}".format(location, days)):  # look if the key is already in the cache table
        return Response(weather_data)  # Return the stored object

    cache.set("{}-{}".format(location, days), weather_data,
